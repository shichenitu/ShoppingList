package dk.verzier.shoppingv8.ui.features.shoppinglist
...
import dk.verzier.shoppingv8.ui.components.ConfirmButton
import dk.verzier.shoppingv8.ui.components.DismissButton
import dk.verzier.shoppingv8.ui.components.NavigationType
import dk.verzier.shoppingv8.ui.components.ShoppingTextField
import dk.verzier.shoppingv8.ui.components.ShoppingTopAppBar
import dk.verzier.shoppingv8.ui.components.ThemedPreviews
import dk.verzier.shoppingv8.ui.navigation.AppRoute
import dk.verzier.shoppingv8.ui.theme.ShoppingV8PreviewTheme
...
private fun AddWhatScreen(
    ...
    var isPizzaDropdownExpanded by remember { mutableStateOf(value = false) }
    ...
    Scaffold(
        ...
        Column(
            ...
            ExposedDropdownMenuBox(
                expanded = isPizzaDropdownExpanded,
                onExpandedChange = { isPizzaDropdownExpanded = !isPizzaDropdownExpanded }
            ) {
                ShoppingTextField(
                    value = uiState.what,
                    ...
                    trailingIcon = if (uiState.what.trim().equals(
                            other = "pizza",
                            ignoreCase = true
                        )) {
                        { ExposedDropdownMenuDefaults.TrailingIcon(expanded = isPizzaDropdownExpanded) }
                    } else null,
                    modifier = if (uiState.what.trim().equals(other = "pizza", ignoreCase = true))
                        Modifier.menuAnchor(
                            type = ExposedDropdownMenuAnchorType.PrimaryEditable,
                            enabled = true
                        ) else Modifier,
                )
                if (uiState.what.trim().equals(other = "pizza", ignoreCase = true)) {
                    ExposedDropdownMenu(
                        expanded = isPizzaDropdownExpanded,
                        ...
                        pizzaToppings.forEach { topping ->
                            DropdownMenuItem(
                                text = { Text(text = topping) },
                                onClick = {
                                    uiEvents.onWhatChange(what = "$topping pizza")
